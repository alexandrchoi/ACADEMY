<%@ Page Language="C#" MasterPageFile="/MasterSub.Master" AutoEventWireup="true" CodeBehind="competency_main.aspx.cs" Inherits="CLT.WEB.UI.LMS.APPR.competency_main" 
...
<script type="text/javascript" language="javascript">
     ...
     {  
        if(!compareDate(document.getElementById('<%=txtSTART_DATE.ClientID %>'),document.getElementById('<%=txtEND_DATE.ClientID %>'),'<%=System.Threading.Thread.CurrentThread.CurrentCulture %>')) return false;
    ...
    {	
        ...
        if(gubun == "new")
            openPopWindow('/appr/competency_detail.aspx?search='+document.getElementById('<%=ddlInquiry.ClientID %>').value+'^'+document.getElementById('<%=ddlVslType.ClientID %>').value+'^'+document.getElementById('<%=ddlShipCode.ClientID %>').value+'^'+document.getElementById('<%=ddlAppDutyStep.ClientID %>').value+'^'+'&MenuCode=<%=Session["MENU_CODE"]%>', "AppForm", "1280", "780", "status=yes");
        else
            openPopWindow('/appr/competency_detail.aspx?search='+rSearch+'&MenuCode=<%=Session["MENU_CODE"]%>', "AppForm", "1280", "780", "status=yes");
	    return false;
    }
</script>
</asp:Content>
...
<div class="section-full">
    <h2 class="page-title"><asp:Label ID="lblMenuTitle" runat="server" Text="평가결과 입력/조회" meta:resourcekey="lblMenuTitle" />
        <!-- 모바일 뒤로 가기 -->
        <span class="goback">
    ...
    <section class="section-board">
        ...
        <div class="button-box right">
            <asp:Button ID="btnNew" runat="server" Text="New" CssClass="button-default blue" OnClientClick="javascript:return GoAppForm('','new');" meta:resourcekey="btnNewResource" />
            <asp:Button ID="btnDel" runat="server" Text="Delete" CssClass="button-default" meta:resourcekey="btnDelResource" OnClick="btnDel_Click" Visible="false" />
            ...
            <C1WebGrid:C1WebGrid ID="grdList" AllowSorting="True" AllowColSizing="True" runat="server" AutoGenerateColumns="false" DataKeyField="app_keys" OnItemDataBound="grdList_ItemDataBound" OnItemCreated="grdList_ItemCreated">
                <Columns>
                   ...
                   <C1WebGrid:C1BoundColumn DataField="re_cnt" HeaderText="C,D항목수">
                    ...
                    <C1WebGrid:C1BoundColumn DataField="APP_COMPLETE" HeaderText="완료 여부">
                        <ItemStyle />
                    </C1WebGrid:C1BoundColumn>
